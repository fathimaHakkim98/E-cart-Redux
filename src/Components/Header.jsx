...
function Header({ insideHome }) {
    ...
    return (
        <div>
            <Navbar style={{ zIndex: '100' }} className="bg-dark text-light position-fixed top-0 w-100">
                <Container>
                    <Navbar.Brand >
                    ...
                    <div>
                        <Link to={'/wishlist'} style={{ textDecoration: 'none' }} className='text-white'>
                            <i className="fa-solid fa-heart pe-1 text-primary"></i>WISHLIST
                            <Badge bg="secondary ms-2">{wishlistCount}</Badge>
                        </Link>
                        <Link to={'/cart'} style={{ textDecoration: 'none' }} className='text-white ps-5'> <i className="fa-solid fa-cart-shopping pe-2 text-white"></i>CART  <Badge bg="secondary ms-2">{cartCount}</Badge></Link>
                    </div>
                </Container>
            </Navbar>
        </div>
